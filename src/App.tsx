...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([taskRepository.list(), categoryRepository.list()]).then(([t, c]) => { setTasks(t); setCategories(c) })
  }, [])
  ...
  async function addTask(title: string, categoryId: string) {
    const task = await taskRepository.create({ title, categoryId, date: selectedDate })
  ...
  async function createCategory(name: string, color: string) {
    const category = await categoryRepository.create({ name, color })
  ...
  return <div className={`app-shell unified-shell ${view !== 'calendar' ? 'without-inspector' : ''}`} data-theme={theme}>
    ...
    <main className={`workspace view-${view}`}>
      ...
      <div className="view-stage" key={view}>
        ...
        {view === 'settings' && <section className="settings-view">
          ...
          <div className="settings-divider" /><div className="settings-section-heading"><Database size={19} /><div><h2>데이터 저장</h2><p>이 브라우저에 저장된 데이터를 관리합니다.</p></div></div><div className="storage-status"><span><i />로컬 저장 사용 중</span><button className="danger-button" onClick={() => setShowReset(true)}><Trash2 size={14} />저장 데이터 삭제</button></div>
        </section>}
    ...
    {showReset && <div className="modal-backdrop"><section className="reset-dialog" role="dialog" aria-modal="true" aria-labelledby="reset-title"><h2 id="reset-title">저장 데이터를 삭제할까요?</h2><p>이 브라우저의 모든 할 일과 이전 루틴 기록을 삭제하고, 카테고리·테마·캘린더 설정을 기본값으로 되돌립니다. 삭제한 데이터는 복구할 수 없습니다.</p><div className="form-actions"><button className="text-button" onClick={() => setShowReset(false)}>취소</button><button className="danger-button solid" onClick={resetData}>삭제하고 초기화</button></div></section></div>}
  </div>
}
...
function TaskPanel({ date, tasks, categories, onAdd, onToggle, onRemove, onCategories }: { date: Date; tasks: Task[]; categories: Category[]; onAdd: (title: string, categoryId: string) => Promise<void>; onToggle: (id: string) => void; onRemove: (id: string) => void; onCategories: () => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (!title.trim() || !activeCategory) return; await onAdd(title.trim(), activeCategory); setTitle(''); setAdding(false) }
...
function Categories({ categories, tasks, onBack, onCreate, onUpdate, onRemove }: { categories: Category[]; tasks: Task[]; onBack: () => void; onCreate: (name: string, color: string) => Promise<void>; onUpdate: (id: string, name: string, color: string) => Promise<void>; onRemove: (id: string) => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (!name.trim()) return; if (editing) await onUpdate(editing, name.trim(), color); else await onCreate(name.trim(), color); setName(''); setEditing(null) }
