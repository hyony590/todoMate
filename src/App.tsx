...
import type { Routine, Task, ThemeId, ViewId } from './types'
...
function App() {
  ...
  useEffect(() => {
    Promise.all([taskRepository.list(), routineRepository.list()]).then(([taskData, routineData]) => {
      setTasks(taskData)
    ...
    <div className="app-shell" data-theme={theme}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <button className={`nav-item ${view === 'settings' ? 'active' : ''}`} onClick={() => switchView('settings')}><Settings size={19} />설정</button>
      ...
      <ContextPanel view={view} selected={selected} selectedDate={selectedDate} selectedTasks={selectedTasks} tasks={tasks} routines={routines} theme={theme} week={week} onSelectDate={setSelectedDate} onOpenDate={() => switchView('today')} />
      ...
      <nav className="mobile-nav" aria-label="모바일 메뉴">
        ...
        <button className={view === 'settings' ? 'active' : ''} onClick={() => switchView('settings')}><Settings size={21} /><span>설정</span></button>
      </nav>
    </div>
  )
}
...
type TodayViewProps = {
  selected: Date; selectedDate: string; selectedTasks: Task[]; progress: number; completedCount: number; week: Date[]; taskCounts: Record<string, number>; isLoaded: boolean; isAdding: boolean; newTask: string; categoryId: string
...
function CalendarView({ date, selectedDate, tasks, onSelect, onMonth }: { date: Date; selectedDate: string; tasks: Task[]; onSelect: (date: string) => void; onMonth: (amount: number) => void }) {
...
function SettingsView({ theme, onTheme }: { theme: ThemeId; onTheme: (theme: ThemeId) => void }) {
  return <section className="settings-view"><div className="settings-section-heading"><Palette size={19} /><div><h2>테마 버전</h2><p>선택한 테마는 이 브라우저에 자동으로 저장됩니다.</p></div></div><div className="theme-options">{themes.map((option) => { const Icon = option.icon; return <button key={option.id} className={`theme-option ${theme === option.id ? 'selected' : ''}`} onClick={() => onTheme(option.id)}><div className={`theme-preview preview-${option.id}`}><span /><span /><span /></div><div className="theme-copy"><Icon size={18} /><span><strong>{option.name}</strong><small>{option.description}</small></span></div><i>{theme === option.id && <Check size={13} strokeWidth={3} />}</i></button> })}</div><div className="settings-divider" /><div className="settings-section-heading"><Database size={19} /><div><h2>데이터 저장</h2><p>현재는 이 기기의 브라우저에 안전하게 저장됩니다.</p></div></div><div className="storage-status"><span><i />로컬 저장 사용 중</span><small>Supabase 연결 준비 완료</small></div><div className="settings-divider" /><div className="settings-section-heading"><CalendarDays size={19} /><div><h2>캘린더 기준</h2><p>주간 화면은 월요일부터 시작합니다.</p></div></div><div className="setting-value"><span>주 시작 요일</span><strong>월요일</strong></div></section>
}

function ContextPanel({ view, selected, selectedDate, selectedTasks, tasks, routines, theme, week, onSelectDate, onOpenDate }: { view: ViewId; selected: Date; selectedDate: string; selectedTasks: Task[]; tasks: Task[]; routines: Routine[]; theme: ThemeId; week: Date[]; onSelectDate: (date: string) => void; onOpenDate: () => void }) {
